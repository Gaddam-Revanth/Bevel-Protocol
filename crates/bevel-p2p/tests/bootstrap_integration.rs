...
async fn test_bootstrap_connectivity() {
    ...
    let boot_addr_final =
        actual_boot_addr.expect("Bootstrap node failed to provide a listen address");
    let boot_peer_id: PeerId = boot_node.peer_id;

    println!(
        "Bootstrap node started at {} with PeerId {}",
        boot_addr_final, boot_peer_id
    );
    ...
    let client_node = BevelNode::new(&client_id, "test_db_client", &bootstraps)
        .await
        .unwrap();
    ...
    assert_ne!(
        client_node.peer_id, boot_peer_id,
        "Client and boot should have different PeerIds"
    );
    assert_eq!(
        client_node.identity().address,
        client_id.address,
...
async fn test_sfp_manifest_signing_integration() {
    ...
    let mut sender_node = BevelNode::new(&sender_id, "test_db_sfp_sender", &[])
        .await
        .unwrap();
    ...
    assert!(
        result.is_ok(),
        "SFP message storage should succeed: {:?}",
        result.err()
    );

    println!(
        "SFP manifest signed and stored successfully for message_id: {}",
...
async fn test_bns_registration_integration() {
    ...
    assert!(
        result.is_ok(),
        "BNS handle registration should succeed: {:?}",
        result.err()
    );

    println!(
        "BNS handle '{}' registered for address '{}'",
        handle, id.address
    );
