...
use bevel_protocol::{
    DmpChunk, DmpContact, DmpMessage, DmpMessageManifest, DmpReputation, DmpSpamPolicy,
};
...
impl BevelDb {
    ...
    pub fn get_identity(
        &self,
        address: &str,
    ) -> Result<Option<BevelIdentity>, Box<dyn std::error::Error>> {
    ...
    pub fn get_ratchet_state(
        &self,
        remote_address: &str,
    ) -> Result<Option<RatchetState>, Box<dyn std::error::Error>> {
        if let Some(ser_state) =
            self.get_item::<SerRatchetState>("ratchets", remote_address.as_bytes())?
        {
    ...
    pub fn get_message(
        &self,
        message_id: &str,
    ) -> Result<Option<DmpMessage>, Box<dyn std::error::Error>> {
        self.get_item("messages", message_id.as_bytes())
    }

    // --- Offline SFP Chunks Cache ---
    pub fn save_sfp_chunk(
        &self,
        chunk_key: &[u8; 32],
        chunk: &DmpChunk,
    ) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub fn get_sfp_chunk(
        &self,
        chunk_key: &[u8; 32],
    ) -> Result<Option<DmpChunk>, Box<dyn std::error::Error>> {
        self.get_item("sfp_chunks", chunk_key)
    }

    // --- Offline SFP Manifest Cache ---
    pub fn save_sfp_manifest(
        &self,
        message_id: &[u8; 32],
        manifest: &DmpMessageManifest,
    ) -> Result<(), Box<dyn std::error::Error>> {
        self.save_item("sfp_manifests", message_id, manifest)
    }

    pub fn get_sfp_manifest(
        &self,
        message_id: &[u8; 32],
    ) -> Result<Option<DmpMessageManifest>, Box<dyn std::error::Error>> {
        self.get_item("sfp_manifests", message_id)
    }

    // --- BNS (Bevel Name Service) Records ---
    pub fn save_bns_record(
        &self,
        record: &bevel_protocol::BnsRecord,
    ) -> Result<(), Box<dyn std::error::Error>> {
        self.save_item("bns_records", record.handle.as_bytes(), record)
    }

    pub fn get_bns_record(
        &self,
        handle: &str,
    ) -> Result<Option<bevel_protocol::BnsRecord>, Box<dyn std::error::Error>> {
    ...
    pub fn get_contact(
        &self,
        address: &str,
    ) -> Result<Option<DmpContact>, Box<dyn std::error::Error>> {
    ...
    pub fn search_messages(
        &self,
        query: &str,
    ) -> Result<Vec<DmpMessage>, Box<dyn std::error::Error>> {
        let tree = self.db.open_tree("search_index")?;
    ...
    pub fn get_reputation(
        &self,
        address: &str,
    ) -> Result<DmpReputation, Box<dyn std::error::Error>> {
    ...
    pub fn save_spam_policy(
        &self,
        policy: &DmpSpamPolicy,
    ) -> Result<(), Box<dyn std::error::Error>> {
...
mod tests {
    ...
    fn test_search_messages() {
        ...
        let msg1 = DmpMessage::new(
            ...
            DmpMessageBody {
                text_plain: "Hello, let's meet at 5pm".to_string(),
                text_html: None,
            },
            DmpMessageFlags {
        ...
        let msg2 = DmpMessage::new(
            ...
            DmpMessageBody {
                text_plain: "Please call me back".to_string(),
                text_html: None,
            },
            DmpMessageFlags {
                request_delivery_receipt: false,
