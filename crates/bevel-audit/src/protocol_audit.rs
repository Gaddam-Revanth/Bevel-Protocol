...
use bevel_protocol::{
    pad_payload, round_timestamp, DmpHeader, DmpMessage, DmpMessageBody, DmpMessageFlags,
};
...
fn test_media_attachment_encryption_isolation() -> Finding {
    ...
    Finding {
        ...
        recommendation:
            "Ensure bevel-media generates a fresh CSPRNG key for every file/folder processed.",
    }
}
...
fn test_timestamp_rounding_10s() -> Finding {
    ...
    let all_correct = samples
        .iter()
        .all(|(raw, expected)| round_timestamp(*raw) == *expected);
    Finding {
        id: "BVL-P01",
        ...
        status: if all_correct {
            Status::Passed
        } else {
            Status::Confirmed
        },
        recommendation:
            "Consider rounding to 60 s for higher-latency anonymous messaging profiles.",
    }
}
...
fn test_payload_padding_masks_length() -> Finding {
    ...
    for sz in [1usize, 50, 400, 1023, 1024] {
        ...
        if padded.len() != 1024 {
            violations.push(format!(
                "input {}B → padded {}B (expected 1024)",
                sz,
                padded.len()
            ));
        }
    ...
    if big.len() != 2048 {
        violations.push(format!(
            "input 1025B → padded {}B (expected 2048)",
            big.len()
        ));
    }
    ...
    Finding {
        ...
        recommendation:
            "Consider using a fixed 64 KB canonical cell size for stronger length hiding.",
    }
}
...
fn test_sfp_sender_masking_placeholder() -> Finding {
    ...
    Finding {
        ...
        recommendation:
            "Ensure sender blinding (HMAC) is computed by the P2P layer before chunking.",
    }
}
    ...
    Finding {
        ...
        description: if rotates {
            "DHT manifest keys rotate based on time epochs. \
             An observer cannot track a recipient's inbox key across epoch boundaries."
                .into()
        } else {
        ...
        recommendation:
            "Rotate the 'inbox token' used for DHT derivation daily (epoch = unix_time / 86400).",
    }
}

/// DmpMessage::new() enforces cryptographically random message IDs.
fn test_message_id_no_uuid_enforcement() -> Finding {
    let body = DmpMessageBody {
        text_plain: "test".into(),
        text_html: None,
    };
    let flags = DmpMessageFlags {
    ...
    Finding {
        ...
        description: if is_random {
            "DmpMessage enforces 128-bit random IDs via its factory method, \
             preventing ordering or count leaks."
                .into()
        } else {
...
fn test_binary_header_fixed_size() -> Finding {
    ...
    Finding {
        ...
        description: if correct {
            format!(
                "DmpHeader serialises to exactly {} bytes — binary format invariant holds.",
                bevel_protocol::HEADER_SIZE
            )
        } else {
            format!(
                "DmpHeader is {}B but HEADER_SIZE={} — binary protocol mismatch!",
                encoded.len(),
                bevel_protocol::HEADER_SIZE
            )
        },
        ...
        recommendation:
            "If header layout changes, update HEADER_SIZE and bump the protocol version.",
    }
}
